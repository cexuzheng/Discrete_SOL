...
class Knowledge_base():
    def __init__(self,u_lim,measurments={'height':0,'roll':0,'pitch':0,'yaw':0},u={'pwm1':0,'pwm2':0,'pwm3':0,'pwm4':0}):
...
class SimResults():
    ...
    def graph(self,j,i):
        ...
        if self.select['states']:
            ...
            # plt.legend(["Control u1","Control u2","Control u3","Control u4","Battery charge", "Capacitor charge"], loc=1)
            # plt.legend(["Control","x1","x2"], loc=1)
            plt.xlabel('t (sec)')
